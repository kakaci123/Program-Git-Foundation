...
public class WekaRandomForest extends Classifiers{
	
	public WekaRandomForest(String[] args) {
		super(args);
		createClassifier();
		if (function==2){
			createClassifier2();
	...
	public void crossValidation(){
		if (function==1) //Use training set
		{
			...
			try{
				WekaRandomForest.setOptions(options);
				WekaRandomForest.buildClassifier(Training);
				Evaluation evaluate = new Evaluation(Training);
				evaluate.evaluateModel(WekaRandomForest, Training);	
...
//		    	System.out.println(evaluate.toMatrixString());
		...
		else if (function==2) //Supplied test set
		{
			...
			try{
				...
				evalf2.evaluateModel(WekaRandomForest, Test);	
...
//		    	System.out.println(evaluate.predictions());
		...
		else if (function==3) //十則驗證
		{
			...
			try{
				...
				WekaRandomForest.buildClassifier(Training);
		    	Evaluation evaluate = new Evaluation(Training);
		    	evaluate.crossValidateModel(WekaRandomForest, Training, cross, new Random(1));
		    	System.out.println(evaluate.toSummaryString("\nResults\n======\n", false));

...
public class WekaSL extends Classifiers{
	
	public WekaSL(String[] args) {
		super(args);
		createClassifier();
		if (function==2){
			createClassifier2();
	...
	public void crossValidation(){
		...
		{
			SimpleLogistic WekaSL = new SimpleLogistic();
			try{
				WekaSL.setOptions(options);
				WekaSL.buildClassifier(Training);
				Evaluation evaluate = new Evaluation(Training);
				evaluate.evaluateModel(WekaSL, Training);	
...
//		    	System.out.println(evaluate.toMatrixString());
		...
		else if (function==2) //Supplied test set
		{
			...
			try{
				...
				evalf2.evaluateModel(WekaSL, Test);	
...
//		    	System.out.println(evaluate.predictions());
		...
		else if (function==3) //十則驗證
		{
			...
			try{
				...
				WekaSL.buildClassifier(Training);
		    	Evaluation evaluate = new Evaluation(Training);
		    	evaluate.crossValidateModel(WekaSL, Training, cross, new Random(1));
		    	System.out.println(evaluate.toSummaryString("\nResults\n======\n", false));

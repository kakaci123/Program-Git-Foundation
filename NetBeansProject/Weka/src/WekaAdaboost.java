...
public class WekaAdaboost extends Classifiers{
	
	public WekaAdaboost(String[] args) {
		super(args);
		createClassifier();
		if (function==2){
			createClassifier2();
	...
	public void crossValidation(){
		...
		{
			AdaBoostM1 WekaAdaboost = new AdaBoostM1();
			try{
				WekaAdaboost.setOptions(options);
				WekaAdaboost.buildClassifier(Training);
				Evaluation evaluate = new Evaluation(Training);
				evaluate.evaluateModel(WekaAdaboost, Training);	
...
//		    	System.out.println(evaluate.toMatrixString());
		...
		else if (function==2) //Supplied test set
		{
			...
			try{
				...
				evalf2.evaluateModel(WekaAdaboost, Test);	
...
//		    	System.out.println(evaluate.predictions());
		...
		else if (function==3) //十則驗證
		{
			...
			try{
				...
				WekaAdaboost.buildClassifier(Training);
		    	Evaluation evaluate = new Evaluation(Training);
		    	evaluate.crossValidateModel(WekaAdaboost, Training, cross, new Random(1));
		    	System.out.println(evaluate.toSummaryString("\nResults\n======\n", false));

...
public class WekaSVM extends Classifiers{
	
	public WekaSVM(String[] args) {
		super(args);
		createClassifier();
		if (function==2){
			createClassifier2();
	...
	public void crossValidation(){
		if (function==1) //Use training set
		{
			SMO WekaSMO = new SMO();
			try{
				WekaSMO.setOptions(options);
				WekaSMO.buildClassifier(Training);
				Evaluation evaluate = new Evaluation(Training);
				evaluate.evaluateModel(WekaSMO, Training);	
...
//		    	System.out.println(evaluate.toMatrixString());
		...
		else if (function==2) //Supplied test set
		{
			...
			try{
				...
				evalf2.evaluateModel(WekaSMO, Test);	
...
//		    	System.out.println(evaluate.predictions());
		...
		else if (function==3) //十則驗證
		{
			...
			try{
				...
				WekaSMO.buildClassifier(Training);
		    	Evaluation evaluate = new Evaluation(Training);
		    	evaluate.crossValidateModel(WekaSMO, Training, cross, new Random(1));
		    	System.out.println(evaluate.toSummaryString("\nResults\n======\n", false));

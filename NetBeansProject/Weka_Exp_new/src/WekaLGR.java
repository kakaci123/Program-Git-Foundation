...
public class WekaLGR extends Classifiers{
	
	public WekaLGR(String[] args) {
		super(args);
		createClassifier();
		if (function==2){
			createClassifier2();
	...
	public void crossValidation(){
		...
		{
			Logistic WekaLGR = new Logistic();
			try{
				WekaLGR.setOptions(options);
				WekaLGR.buildClassifier(Training);
				Evaluation evaluate = new Evaluation(Training);
				evaluate.evaluateModel(WekaLGR, Training);	
...
//		    	System.out.println(evaluate.toMatrixString());
		...
		else if (function==2) //Supplied test set
		{
			...
			try{
				...
				evalf2.evaluateModel(WekaLGR, Test);	
...
//		    	System.out.println(evaluate.predictions());
		...
		else if (function==3) //十則驗證
		{
			...
			try{
				...
				WekaLGR.buildClassifier(Training);
		    	Evaluation evaluate = new Evaluation(Training);
		    	evaluate.crossValidateModel(WekaLGR, Training, cross, new Random(1));
		    	System.out.println(evaluate.toSummaryString("\nResults\n======\n", false));

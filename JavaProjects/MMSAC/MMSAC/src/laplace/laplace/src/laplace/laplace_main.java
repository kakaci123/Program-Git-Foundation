package laplace.laplace.src.laplace;
...
public class laplace_main {

	ArrayList TESTSETPOINTER, FREQDATACASELIST;
	int classcount = 0;

	public laplace_main(String filename, int foldint) {
		double[] temp_result = new double[foldint];
		double result = 0.0;
		for (int i = 0; i < temp_result.length; i++) {
			temp_result[i] = new laplace_main("output/output" + (i + 1) + ".txt",
	...
	public laplace_main(String patternfile, String testfile) {
		...
		try {
			...
			while ((line_data = buff_reader.readLine()) != null) {
				...
				if (str_token.countTokens() == 1) { // 當token數目為一,表示此行為class
					...
					if (id == tempid) {// 同一個id表示接收的資料行屬於同一datacase,故不需要new
										// datacase
						if (time == 0) {
					...
					} else {// 不同id表示已經讀到下一個datacase,此時要將上一個datacase連結到該class
							// label的subDB
						id = tempid;
						tempdatacase.setClassLabel(classlabel);
						subdb.add(tempfreqdatacase);
	...
	public double scoring() {
		...
		for (int i = 0; i < TESTSETPOINTER.size(); i++) {
			...
			for (int j = 0; j < subtestdb.size(); j++) {
				testsetcounter++;
				short testdclabel = ((datacase) subtestdb.get(j)).getClassLabel();// 取得目前test
																					// datacase的class
																					// label
...
//		System.out.println((double) accuracy / testsetcounter);
	...
	public short decideClassLabelByScoring(datacase testdc) {
		...
		for (int i = 0; i < FREQDATACASELIST.size(); i++) {
			...
			score += (double) (counter + 1) / (double) (totalCounter + classcount);

			// System.out.println("class: " + tempclass + ". LaplaceAccuracy: "
			// + score);
			if (score > finalscore) {
				finalscore = score;

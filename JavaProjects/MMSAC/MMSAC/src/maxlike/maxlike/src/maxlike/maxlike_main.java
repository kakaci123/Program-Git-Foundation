...
public class maxlike_main {

	ArrayList TESTSETPOINTER, FREQDATACASELIST;

	public maxlike_main(String filename, int foldint) {
		double[] temp_result = new double[foldint];
		double result = 0.0;
		for (int i = 0; i < temp_result.length; i++) {
			temp_result[i] = new maxlike_main("output/output" + (i + 1) + ".txt",
	...
	public maxlike_main(String patternfile, String testfile) {
	...
	public double scoring() {
		...
		// (double)accuracy/testsetcounter );
		...
		for (int i = 0; i < FREQDATACASELIST.size(); i++) {
			...
			for (int j = 0; j < subdb.size(); j++) {
				...
				if (isContain(freqdc.getDatacase(), testdc)) {
					score += freqdc.getConfidence(); // Confidence 最大
				}
			}
			// System.out.println("class: " + tempclass + ". Accuracy: " +
			// score);
			if (score > finalscore) {
				finalscore = score;
